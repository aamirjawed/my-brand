...
export default function BrandingHero() {
  const benefits = ["Brand Identity", "Logo Systems", "Visual Language", "Brand Recognition", "Market Authority"];
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!isDeleting) {
        ...
        setDisplayText(current.substring(0, displayText.length - 1));
        if (displayText.length === 0) { setIsDeleting(false); setIndex((p) => (p + 1) % benefits.length); }
      }
    }, typeSpeed);
    ...
    <section className="relative h-auto pt-44 pb-28 sm:py-36 md:py-40 flex flex-col overflow-hidden bg-slate-900">
      <div className="absolute inset-0 z-0">
        ...
        <img
          ...
          alt="Branding and Logo Design Services"
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed max-w-xl mx-auto mb-10">
              A powerful brand is your most valuable business asset. We craft complete brand identities and iconic logo systems that command attention, build trust, and make you permanently unforgettable.
            </p>
          </motion.div>
        </div>
      </div>
